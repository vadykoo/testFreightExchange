<template>
    <div class="container">
        <a href="/api/generate" target="_blank" class="btn btn-success">Згенерувати вантаж для перевірки роботи
            вебсокет </a>
            ...
            <div class="col-md-12">
                ...
                <div class="card" v-for="(load, index) in loads">
                    <div class="card-header" v-on:click="setCurrent(index)">
                        <div class="row">
                            <div class="col-8 font-weight-bold">{{load.city_from.name}} - {{load.city_to.name}}</div>
                    ...
                    <div class="card-body" v-if="index === currentLoad">
                        <div class="row">
                            <div class="col">
                                <h2 class="font-weight-bold">{{load.name}}</h2>
                            </div>
                        </div>
                        <div class="row">
                            ...
                            <div class="col-6">
                                <iframe width="100%" height="100%" frameborder="0"
                                        v-bind:src="src(load.city_from.slug, load.city_to.slug)"></iframe>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    export default {
        ...
        methods: {
            fetchLoads(city_from) {
                let page_url = '/api/' + this.$i18n.locale +'/loads';
                if (city_from) {
            ...
            src: function (origin, destination) {
                return 'https://www.google.com/maps/embed/v1/directions?key=' + process.env.MIX_APP_GOOGLE_MAPS_KEY + '&language=' + this.$i18n.locale + "&origin=" + origin + "&destination=" + destination;
            }
        }
    }
</script>
